...
You need the **devtools** package to install **findPvalueCutoff**. This can be done by installing devtools by typing `r colFmt("install.packages(devtools)",'darkgreen')` command in R. If the **devtools** package is already installed, then you can use the following command to load **devtools** package in the current R environment.  
